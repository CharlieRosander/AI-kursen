...
home_data_ocean['ocean_proximity'] = home_data_ocean['ocean_proximity'].map(
...
X_train, X_test, y_train, y_test = train_test_split(home_data_median[['latitude', 'longitude']],
                                                    home_data_median[['median_house_value']], test_size=0.33,
                                                    random_state=0)
...
# We see that the optimal number of clusters seems to be 5, as the silhouette score is highest for k = 5 without
# overfitting, which k = 7 and k = 8 seems to do.
